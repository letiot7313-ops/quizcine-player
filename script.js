...
function connect(){
  const stored = localStorage.getItem('ws-server') || DEFAULT_WS || prompt('URL serveur WebSocket');
  ...
  socket.on('reveal', data=>{ accepting=false; updateStatus('Révélé.'); });
...
function showQuestion(q){
  ...
  const img=$('qimg'), vid=$('qvid');
  if(q.image){ img.src=q.image; img.style.display='block'; } else { img.style.display='none'; }
  if(q.video){ vid.src=q.video; vid.style.display='block'; try{ vid.play().catch(()=>{});}catch(e){} } else { vid.pause?.(); vid.style.display='none'; }
  ...
  if(!isOpen){
    ...
    $('sendOpen').onclick=()=>{ const v=($('openAnswer').value||'').trim(); if(!v) return; sendAnswer(v,$('sendOpen')); };
  }
...
function sendAnswer(val,node){
  if(!accepting) return;
  if(!allowChange){
    const already=document.querySelector('.choice.selected'); if(already) return;
  }
...
const codeFromUrl=qs('code'); if(codeFromUrl){ $('code').value=codeFromUrl.toUpperCase(); }
